...
public:
   ...
   bool operator()(GenotypeT& _genotype1, GenotypeT & _genotype2)
  {
      ...
      ordoner( ordre1,completion1, Sd[0], Sf[0], _genotype1, _genotype2,_genotype1.getCompletionTime(Sd[0]));
      ordoner( ordre2,completion2, Sd[1], Sf[1], _genotype2, _genotype1,_genotype2.getCompletionTime(Sd[1]));
    ...
    void ordoner(vector<int> & ordre,vector<int> & completion, int Sd, int Sf, GenotypeT & _genotype1,GenotypeT & _genotype2,int Tini)
    {
       // cout<<"deb ord "<<Sd<<" "<<Sf<<endl;
        ...
        while(nbAd<Sf-Sd)
        {
            ...
            if(!_genotype1.getDejaVu(Sd,job))
            {
                ...
                int t;
                if(nbAd==0)t=Tini+p;
                else t=completion[nbAd-1]+p;
                if(t<r+p) t=r+p;
                if(t<_genotype2.getCompletionTime(i) && t<d)
                {
                    t=_genotype2.getCompletionTime(i);
                    if(t>d)
                        t=d;
                }
    ...
    int chercheSommetFinal(int & Sd,GenotypeT & _genotype1,GenotypeT & _genotype2)
    {
        ...
        {
             //cout<<"fin chercher1 "<<endl;//<<Tmin<<" "<<_genotype1.getCompletionTime(Sd)<<" "<<Sf<<" "<<j-1<<" "<<dejaVus[j-1] << _genotype2.getDejaVu(Sf,j-1)<<" completion time "<<_genotype2.getCompletionTime(Sf)<<endl;
            return Sf;
        }
        else
        {
